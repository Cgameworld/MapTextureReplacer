...
export const OpenImage = (index: number, path: string) => trigger(GROUP, "open_image", index, path);
export const ResetTexture = (index: number) => trigger(GROUP, "reset_texture", index);
export const ChangeFloatField = (name: string, value: number) => trigger(GROUP, "change_float_field", name, value);
export const ResetTiling = (group: string) => trigger(GROUP, "reset_tiling", group);
